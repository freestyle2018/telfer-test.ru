...
class ModelExtensionFeedBranchedSitemap extends Model {
	public function getCategories($data = []) {
		...
		if ($this->config->get('branched_sitemap_multishop')) {
  ...
  public function getProducts($data = array()) {
    ...
    if (!$this->config->get('branched_sitemap_off_description') || $this->config->get('branched_sitemap_require_image_caption')) {
      $sql .= ", pd.name";
    }
  ...
  public function getTotalProducts() {
    ...
    if (!$this->config->get('branched_sitemap_off_description')) {
      $sql .= " LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id)";
    }
  ...
  public function getManufacturers($data = []) {
    $system = $this->config->get('branched_sitemap_system');
